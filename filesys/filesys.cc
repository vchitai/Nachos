...
{ 
    ...
    if (sector >= 0)        
    this->openFile[id] = new OpenFile(sector);  // name was found in directory 
    id++;
    delete directory;
    return this->openFile[id - 1];              // return NULL if not found
}
...
OpenFile* FileSystem::Open(char *name, int type) {
    ...
    sector = directory->Find(name); 
    if (sector >= 0)        
    this->openFile[id] = new OpenFile(sector, type);    // name was found in directory 
    id++;
    delete directory;
    return this->openFile[id - 1];                // return NULL if not found
}
...
int FileSystem::closeFile(int fileID) {
...
int FileSystem::isOpened(int fileID) {
      if (fileID < 0 || fileID >= id)
        return -1;
      if (this->openFile[fileID] == NULL)
        return -1;
